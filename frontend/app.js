...
function renderAgentEvidence(items = []) { let panel = $('agentEvidence'); if (!panel) { panel = document.createElement('details'); panel.id = 'agentEvidence'; panel.className = 'answer-sources'; $('answer').parentElement?.appendChild(panel); } const detail = (item) => { if (item.status === 'computed') return `${item.expression} = ${item.value}`; if (item.status === 'counterexample_found') return `反例：${JSON.stringify(item.counterexample)}`; if (item.status === 'formally_verified') return `定理 ${item.theorem || ''} 已通过 Lean 检查`; if (item.status === 'needs_clarification') return `缺少参数：${(item.missing_parameters || item.missing || []).join('、')}`; return item.detail || item.assumptions || JSON.stringify(item); }; panel.innerHTML = `<summary>受控验证证据 ${items.length ? `· ${items.length} 条` : '· 未启用'}</summary>${items.length ? items.map(item => `<article><b>${escapeHtml(item.evidence_type || item.status || '证据')}</b><small>${escapeHtml(item.status || '')} · ${escapeHtml(item.method || item.planned_action || '受控动作')}</small><p>${escapeHtml(detail(item))}</p>${item.assumptions ? `<p class="muted">范围：${escapeHtml(item.assumptions)}</p>` : ''}</article>`).join('') : '<span class="muted">本轮未启用受控验证，回答数学正确性仍未验证。</span>'}`; }
async function renderAgentTaskEvents(requestId) { let panel = $('agentTaskEvents'); if (!panel) { panel = document.createElement('details'); panel.id = 'agentTaskEvents'; panel.className = 'answer-sources'; $('answer').parentElement?.appendChild(panel); } if (!requestId) { panel.innerHTML = '<summary>Agent 过程 · 未记录</summary>'; return; } try { const response = await fetch(`${API}/ai/tasks/${encodeURIComponent(requestId)}/events`); const data = await response.json(); if (!response.ok) throw new Error(data.detail || '任务事件读取失败'); panel.innerHTML = `<summary>Agent 过程 · ${data.total || 0} 个阶段</summary>${(data.items || []).map(item => `<article><b>${escapeHtml(item.stage)}</b><small>${escapeHtml(item.status)} · ${escapeHtml(item.created_at || '')}</small>${item.detail ? `<p>${escapeHtml(item.detail)}</p>` : ''}</article>`).join('') || '<span class="muted">暂无任务事件。</span>'}`; } catch (error) { panel.innerHTML = `<summary>Agent 过程 · 暂不可用</summary><span class="muted">${escapeHtml(error.message)}</span>`; } }
...
async function ask(event) {
  ...
  try {
    ...
    renderAgentEvidence(data.agent_tool_evidence);
    renderAgentTaskEvents(data.request_id);
    ...
    if (error.name === 'AbortError') {
      ...
      if (recovered) {
        ...
        renderAgentEvidence(recovered.agent_tool_evidence);
        renderAgentTaskEvents(recovered.request_id || lastAskRequestId);
...
async function enqueueGraphRetry(question, conversationId = null) { try { const response = await fetch(`${API}/ai/retry-queue`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ operation: 'related_graph', question, conversation_id: conversationId, priority: 10 }) }); const job = await response.json(); if (response.ok) { showMessage(`本次生成失败，已加入高优先级重试队列（任务 ${job.id}）。正在等待结果…`); pollGraphRetry(job.id); } } catch (error) {} }
...
async function generateRelatedGraph() {
  ...
  try {
    ...
    enqueueGraphRetry(question, activeConversationId || null);
  } finally {
...
async function loadCandidate(candidateId) { try { const response = await fetch(`${API}/ai/graph-candidates/${candidateId}`); const data = await response.json(); if (!response.ok) throw new Error(data.detail || '候选图谱读取失败'); activeCandidateId = data.id; activeGraphEvidence = data.evidence || []; $('question').value = data.question; $('graphEditor').value = JSON.stringify(data.knowledge_graph, null, 2); $('graphEditorPanel').open = false; $('saveGraphButton').classList.toggle('hidden', data.status === 'saved'); renderGraph(data.knowledge_graph); loadCandidateEvents(candidateId); showMessage(`已载入候选图谱：${data.status}`); } catch (error) { showMessage(`无法载入候选图谱：${error.message}`); } }
...
async function loadAnswerReviews() { let panel = $('answerReviews'); if (!panel) { panel = document.createElement('details'); panel.id = 'answerReviews'; panel.className = 'retry-jobs'; panel.innerHTML = '<summary>人工复核队列</summary><div class="template-list"></div>'; $('graph').appendChild(panel); } try { const data = await (await fetch(`${API}/ai/reviews?status=pending&limit=12`)).json(); panel.querySelector('.template-list').innerHTML = (data.items || []).map(item => `<article class="answer-review-item" data-answer-review="${item.id}"><div class="template-item"><span><b>回答 #${item.answer_id}</b><small>${escapeHtml(item.note || '待核对')}</small></span><span class="review-actions"><button type="button" data-review-status="fixed">确认保存</button><button type="button" data-review-status="false_positive">标记误报</button></span></div><div class="answer-review-fields"><label>总体<select data-review-field="correctness"><option value="unverified">未判断</option><option value="correct">正确</option><option value="partially_correct">部分正确</option><option value="incorrect">错误</option></select></label><label>结论<select data-review-field="conclusion"><option value="unverified">未判断</option><option value="correct">正确</option><option value="partially_correct">部分正确</option><option value="incorrect">错误</option></select></label><label>条件<select data-review-field="conditions"><option value="unverified">未判断</option><option value="correct">正确</option><option value="partially_correct">部分正确</option><option value="incorrect">错误</option></select></label><label>推理<select data-review-field="reasoning"><option value="unverified">未判断</option><option value="correct">正确</option><option value="partially_correct">部分正确</option><option value="incorrect">错误</option></select></label><label>引用<select data-review-field="citations"><option value="unverified">未判断</option><option value="correct">正确</option><option value="partially_correct">部分正确</option><option value="incorrect">错误</option></select></label></div><textarea data-review-note placeholder="审核备注：指出错误步骤、缺失条件或引用问题（可选）"></textarea></article>`).join('') || '<span class="muted">暂无待复核答案</span>'; panel.querySelectorAll('[data-review-status]').forEach(button => button.onclick = async () => { const item = button.closest('[data-answer-review]'); const payload = { status: button.dataset.reviewStatus, note: item.querySelector('[data-review-note]').value.trim() || null }; item.querySelectorAll('[data-review-field]').forEach(field => { payload[field.dataset.reviewField] = field.value; }); button.disabled = true; try { const response = await fetch(`${API}/ai/answers/${item.querySelector('b').textContent.replace('回答 #', '')}/review`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)}); const result = await response.json(); if (!response.ok) throw new Error(result.detail || '保存失败'); loadAnswerReviews(); loadAnswerStats(); } catch (error) { button.disabled = false; showMessage(`答案复核失败：${error.message}`); } }); } catch (error) { panel.querySelector('.template-list').textContent = '复核队列暂不可用'; } }
...
async function openGraphConceptCard(concept) {
  ...
  const relationMarkup = relatedEdges.length
    ? relatedEdges.map(edge => {
      ...
      const subjectKey = `${edge.source}->${edge.target}:${relation}`;
      const evidence = activeGraphEvidence.find(item => item.subject_type === 'edge' && item.subject_key === subjectKey);
      const relationSources = evidence?.source_message_id ? `消息 ${evidence.source_message_id}` : (conversationMatches(`${concept.name} ${other?.name || ''}`).length ? '当前对话直接提及' : '模型推断');
      return `<span class="graph-source-relation"><b>${escapeHtml(relation)}</b> → ${escapeHtml(other?.name || String(otherId))}<small>${escapeHtml(relationSources)}</small>${evidence?.excerpt ? `<em>${escapeHtml(evidence.excerpt.slice(0, 120))}</em>` : ''}</span>`;
    }).join('')
    : '<span>暂无当前图谱关系</span>';
  card.classList.remove('hidden');
  const evidenceMarkup = nodeEvidence.length ? nodeEvidence.map(item => `<blockquote><small>消息 ${escapeHtml(String(item.source_message_id || ''))} · ${escapeHtml(item.review_status || '未审核')}</small>${escapeHtml(item.excerpt || '')}</blockquote>`).join('') : '';
  card.innerHTML = `<div class="graph-card-head"><div><span class="graph-card-tag">${escapeHtml(concept.type || 'concept').toUpperCase()} · ${escapeHtml(concept.field || '数学知识')}</span><h4>${escapeHtml(concept.name)}</h4></div><button class="graph-card-close" type="button" aria-label="关闭概念卡片">×</button></div><p>${escapeHtml(concept.description || '该知识点暂无说明。')}</p><div class="graph-card-sources"><b>对话出处</b><span>${nodeEvidence.length ? `已保存 ${nodeEvidence.length} 条证据` : (sourceMessages.length ? `当前对话中出现 ${sourceMessages.length} 条消息` : '当前对话未直接提及，由图谱关系推断')}</span>${evidenceMarkup}</div><div class="graph-card-relations"><b>关系来源</b><div>${relationMarkup}</div></div><div class="graph-card-path"><b>推荐学习顺序</b><small class="learning-evidence-note">掌握状态只由审核后的作答证据更新，不再通过点击手动标记。</small><span>正在生成路径…</span></div><div class="graph-card-links"><span>正在读取关联知识点…</span></div>`;
...
async function requestConversationStream(question, signal) {
  const response = await fetch(`${API}/conversations/${activeConversationId}/messages/stream`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: question, agent_mode: Boolean($('agentMode')?.checked) }), signal });
