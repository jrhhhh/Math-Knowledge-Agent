...
async function ask(event) { event.preventDefault(); const question = $('question').value.trim(); if (!question) return; clearMessage(); askController = new AbortController(); const timeout = setTimeout(() => askController.abort(), 90000); $('askButton').disabled = true; $('askButton').innerHTML = '正在推理…'; $('cancelAskButton').classList.remove('hidden'); $('answerStatus').textContent = '生成中'; try { const response = await fetch(`${API}/ai/ask`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question }), signal: askController.signal }); const data = await response.json(); if (!response.ok) throw new Error(data.detail || '请求失败'); renderAnswer(data.answer); renderConcepts(data.concepts); $('answerStatus').textContent = data.answer_source === 'deepseek_extended' ? '深入生成完成' : '已完成'; if (data.knowledge_graph) renderGraph(data.knowledge_graph); } catch (error) { if (error.name === 'AbortError') { showMessage('推理已取消或超过 90 秒，当前连接已释放。'); $('answerStatus').textContent = '已取消'; } else { showMessage(`暂时无法连接 Math Agent：${error.message}`); $('answerStatus').textContent = '连接失败'; } } finally { clearTimeout(timeout); askController = null; $('askButton').disabled = false; $('askButton').innerHTML = '开始推理 <span>→</span>'; $('cancelAskButton').classList.add('hidden'); } }
