...
async function ask(event) { event.preventDefault(); const question = $('question').value.trim(); if (!question) return; clearMessage(); askController = new AbortController(); const timeout = setTimeout(() => askController.abort(), 90000); $('askButton').disabled = true; $('askButton').innerHTML = '正在推理…'; $('cancelAskButton').classList.remove('hidden'); $('answerStatus').textContent = '生成中'; try { const data = await requestAskStream(question, askController.signal); renderAnswer(data.answer); renderConcepts(data.concepts); const sourceNames = { deepseek_extended: '深入生成完成', backup_model: '备用模型生成完成', local_fallback: '本地兜底生成完成' }; $('answerStatus').textContent = sourceNames[data.answer_source] || (data.cache_hit ? '缓存命中' : '已完成'); if (data.knowledge_graph) renderGraph(data.knowledge_graph); } catch (error) { if (error.name === 'AbortError') { showMessage('推理已取消或超过 90 秒，当前连接已释放。'); $('answerStatus').textContent = '已取消'; } else { showMessage(`暂时无法连接 Math Agent：${error.message}`); $('answerStatus').textContent = '连接失败'; } } finally { clearTimeout(timeout); askController = null; $('askButton').disabled = false; $('askButton').innerHTML = '开始推理 <span>→</span>'; $('cancelAskButton').classList.add('hidden'); } }
