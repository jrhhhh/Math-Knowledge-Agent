...
class TemplateAPIContractTests(unittest.TestCase):
    ...
    def test_sample_recommendations_contract(self):
        response = self.client.get("/local-templates/samples/recommendations")
        self.assertEqual(response.status_code, 200)
        self.assertIn("length_buckets", response.json())

    def test_sample_cleanup_contract(self):
        response = self.client.delete("/local-templates/samples?retention_days=3650")
        self.assertEqual(response.status_code, 200)
