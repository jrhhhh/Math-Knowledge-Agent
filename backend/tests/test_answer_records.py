...
class AnswerRecordContractTests(unittest.TestCase):
    ...
    def test_answer_list_contract(self):
        ...
        for name in ("math_agent_requests_total", "math_agent_cache_hits_total", "math_agent_backups_failed_total", "math_agent_success_ratio"):
            self.assertRegex(metrics.text, rf"(?m)^{name}(?:\{{[^}}]+\}})?\s+-?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?$")
    ...
    def test_feedback_contract_for_saved_answer(self):
        ...
        try:
            ...
            reviewed = self.client.post(f"/ai/answers/{item.id}/review", json={"status": "false_positive", "note": "已核对", "correctness": "correct", "conclusion": "correct", "conditions": "correct", "reasoning": "partially_correct", "citations": "unverified"})
            self.assertEqual(reviewed.status_code, 200)
            self.assertEqual(reviewed.json()["status"], "false_positive")
            self.assertEqual(reviewed.json()["correctness"], "correct")
            stats = self.client.get("/ai/answers/stats").json()
            self.assertIn("correctness_counts", stats)
