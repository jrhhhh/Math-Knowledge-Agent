...
仓库中的 `data/answer_benchmark.chapter1.json` 是第 1 章的人工审核基准题集，目前包含覆盖直接证明、逆反命题、反证法、归纳法、反例、缺失条件和引用的代表案例。运行 `python3 scripts/validate_answer_benchmark.py` 可检查题目 ID、评分维度和审核标签结构；该脚本只验证基准数据完整性，不会把模型互评冒充数学证明。
