...
数据库备份：执行 `./scripts/backup_db.sh` 使用 SQLite 在线备份生成 `backups/math_agent-时间.db`，默认保留 14 天（可用 `MATH_AGENT_BACKUP_RETENTION_DAYS` 调整）；恢复前请停止后端，执行 `./scripts/restore_db.sh backups/xxx.db`，脚本会先执行 `PRAGMA integrity_check`。可用 cron 每日执行，例如 `0 3 * * * cd /path/to/Math-Agent && ./scripts/backup_db.sh >> /tmp/math-agent-backup.log 2>&1`。
