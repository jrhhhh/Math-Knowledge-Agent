...
数据库备份：执行 `./scripts/backup_db.sh` 使用 SQLite 在线备份生成 `backups/math_agent-时间.db`；恢复前请停止后端，执行 `./scripts/restore_db.sh backups/xxx.db`，脚本会先执行 `PRAGMA integrity_check`。
